...
(Sketch of proof) By first iso theorem, WTS $R/\\Ker{\\varphi}$ is local. Know $1_R \\notin \\Ker{\\varphi}$ as that would imply $\\Ker{\\varphi} = R$, hence $\\Ima{\\varphi} = 0$, a contradiction. Now, since $\\Ima{\\varphi} \\neq 0$, we clearly have $R \\neq 0$; therefore every ideal of $R$, such as $\\Ker{\\varphi}$, is contained in a maximal ideal of $R$. But $R$ being local has a unique maximal ideal, call it $M$, whence $\\Ker{\\varphi} \\subseteq M$. Finally, we apply the correspondence theorem (AKA the fourth isomorphism theorem), which says that the ideals that contain $\\Ker{\\varphi}$ are in bijection with the ideals of $R/\\Ker{\\varphi}$, hence every ideal of $R/\\Ker{\\varphi} \\cong \\Ima{\\varphi}$ is of the form $J/\\Ker{\\varphi}$. Any such ideal must be contained in $M/\\Ker{\\varphi}$ or be $\\Ima{\\varphi}$ itself, hence $M\\Ker{\\varphi}$ is the unique maximal ideal, and so $\\Ima{\\varphi}$ is local. $\\blacksquare// To add a new post, copy the format below and add it to the top of the list.
// The date is optional but recommended.
